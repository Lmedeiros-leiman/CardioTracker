...
export class Cpf extends EntityId<"Cpf"> {
    ...
    /** Para rehidratação do banco — dígitos puros já validados anteriormente. */
    static fromTrusted(digits: string): Cpf {
    ...
    private static readonly checkFormat = (input: string): Result<string, CpfError> => {
        if (/^\d+$/.test(input)) return err({ kind: "missing_mask", input });
    ...
    private static readonly checkDigits = (digits: string): Result<string, CpfError> => {
        if (/^(\d)\1{10}$/.test(digits)) return err({ kind: "invalid_characters", input: digits });
        if (!Cpf.verifyDigit(digits, 9) || !Cpf.verifyDigit(digits, 10)) return err({ kind: "invalid_characters", input: digits });
        return ok(digits);
    };
